...
def filter_seqs_by_header(
    input_fastq_path, header_patterns_path, out_fastq_path, fp_out
):
    try:
    ...
    filter_headers = [
        line.strip() for line in filter_headers if (len(line.strip()) > 0)
    ]
    fp_filter.close()
    ...
    while True:
        ...
        for filter_header in filter_headers:
            if (
                filter_header[-1] == ";"
                and filter_header[0:-1].lower() == header.lower()
            ) or (filter_header[-1] != ";" and filter_header.lower() in header.lower()):
                num_matches += 1
...
def filter_seqs_by_read_id(input_fastq_path, read_id_path, out_fastq_path, fp_out):
    ...
    fp_filter.close()
    filter_read_ids = [
        int(line.strip()) for line in filter_read_ids if (len(line.strip()) > 0)
    ]
    id_hash = {}
...
def filter_seqs_by_header_list(
    input_fastq_path, filter_headers, out_fastq_path, fp_out
):
    try:
        fp_in = open(input_fastq_path, "r")
    except:
...
def filter_seqs_by_length(
    input_fastq_path, length_threshold, is_less_than, out_fastq_path, fp_out
):
    try:
        fp_in = open(input_fastq_path, "r")
    except:
    ...
    while True:
        ...
        if (is_less_than == False and len(read[1]) >= length_threshold) or (
            is_less_than == True and len(read[1]) <= length_threshold
        ):
            num_matches += 1
            if (i % 1000) == 0:
                sys.stderr.write('\rFound %d seqs, last: "%s".' % (num_matches, header))
...
def base_quality_stats(input_fastq_path):
    ...
    if len(means_1d) > 0:
        sys.stdout.write(
            "[1d] avg = %5.2f\tstd = %5.2f\tmin = %2d\tmax = %2d\n"
    ...
    if len(means_2d) > 0:
        sys.stdout.write(
            "[2d] avg = %5.2f\tstd = %5.2f\tmin = %2d\tmax = %2d\n"
...
def base_quality_filter(
    input_fastq_path, lte_gte, qv_threshold, out_fastq_path, fp_out
):
    ...
    while True:
        if (num_reads % 1000) == 0:
            sys.stderr.write(
                "\rProcessing seq %d, (%d passed)..." % (num_reads, num_outputted_reads)
            )
        ...
        if (
            (lte_gte == "gt" and mean_qv > qv_threshold)
            or (lte_gte == "gte" and mean_qv >= qv_threshold)
            or (lte_gte == "lt" and mean_qv < qv_threshold)
            or (lte_gte == "lte" and mean_qv <= qv_threshold)
            or (lte_gte == "eq" and mean_qv == qv_threshold)
        ):
    ...
    sys.stderr.write(
        "Number of sequences not satisfying condition: %d\n" % (num_skipped_reads)
    )
    sys.stderr.write("\n")
    fp_in.close()
...
def hard_clip(input_fastq_path, num_bases):
    ...
    sys.stderr.write(
        "Trimming %dbp from the beginning and ending of each sequence.\n" % num_bases
    )
...
def remove_special_chars_from_headers(input_fastq_path, out_fastq_path, fp_out):
    ...
    while True:
        ...
        read[0] = read[0][0] + re.sub("[^0-9a-zA-Z]", "_", read[0][1:])
        # re.sub("[|:", "_", read[0][1:]);
        fp_out.write("\n".join(read) + "\n")
...
def filter_for_marginalign(input_fastq_path, out_fastq_path, fp_out):
    ...
    if fp_out == None:
        ...
        try:
            ...
            sys.stderr.write(
                'ERROR: Could not open file "%s" for writing! Exiting.\n'
                % out_fastq_path
            )
            exit(0)
    ...
    while True:
        ...
        if len(read[1]) <= 50000:
            ...
            new_header = read[0][0] + re.sub("[^0-9a-zA-Z]", "_", read[0][1:])
            # re.sub("[|:", "_", read[0][1:]);
...
def wrap_fastq_file(input_fastq_path, wrap_length, out_fastq_path, fp_out):
    ...
    while True:
        ...
        ### Wrap the sequence line, because DALIGNER has a 9998bp line len limit.
        if len(read) == 4:
            read[3] = re.sub("(.{%d})" % (wrap_length), "\\1\n", read[3], 0, re.DOTALL)
            ### Wrap the qual line, because DALIGNER has a 9998bp line len limit.
        fp_out.write("\n".join(read) + "\n")
...
def separate_seqs(input_fastq_file, out_folder, headers_as_fn=False):
    ...
    for i in range(len(seqs)):
        if headers_as_fn == False:
            ...
            fp = open(
                "%s/%s.fast%c"
                % (out_folder, headers[i].split()[0], input_fastq_file[-1]),
                "w",
            )
...
def convert_reads_to_pacbio_format(reads_file, pacbio_reads_file):
    ...
    while True:
        ...
        if (
            len(read[1]) <= 50
        ):  ### DALIGNER has a lower length limit of 'k'. Trim more just to be safe.
            sys.stderr.write(
                "Found a read shorter than 10bp. Removing from the output.\n"
            )
            continue
        ...
        read[0] = "%s%s" % (read[0][0], pacbio_header)
        ### Keep the first char of the header line.
        read[1] = re.sub("(.{500})", "\\1\n", read[1], 0, re.DOTALL)
        ### Wrap the sequence line, because DALIGNER has a 9998bp line len limit.
        if len(read) == 4:
            read[3] = re.sub("(.{500})", "\\1\n", read[3], 0, re.DOTALL)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "header":
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            sys.stderr.write(
                "Takes a FASTA/FASTQ file and a file containing header patterns. Extracts all sequences containing the specified header patterns.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_pattern_file> <input_fastq_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            sys.stderr.write(
                "\tIf any of the patterns match a substring of a header, the sequence will be outputted.\n"
            )
            sys.stderr.write(
                '\tTo match the entire header and not only a substring, place a semicolon ";" character at the end of the pattern line.\n'
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) == 5:
            ...
            if input_fastq_path == out_fastq_path:
                sys.stderr.write(
                    "ERROR: Output and input files are the same! Exiting.\n"
                )
                exit(0)
            try:
                fp_out = open(out_fastq_path, "w")
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            sys.stderr.write(
                "Filters the duplicate FASTA/FASTQ entries. Duplicates are equal entries up to the first space. (E.g. gi|49175990|ref|NC_000913.2|)\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            sys.stderr.write(
                "If a FASTA/FASTQ entries are stored in lines of specific width, this tool joins them all into one line."
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<out_filtered_fastq_file>]\n"
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            ...
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s min_len <input_fastq_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        filter_seqs_by_length(
            input_fastq_path, min_length, False, out_fastq_path, fp_out
        )
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            ...
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s max_len <input_fastq_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        filter_seqs_by_length(
            input_fastq_path, max_length, True, out_fastq_path, fp_out
        )
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 3:
            sys.stderr.write(
                "This is not an actual filter, but calculates statistics of the base qualities provided in the FASTQ file.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file>\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 4:
            sys.stderr.write(
                "Trim each sequence from the left and the right end by a given amount of bases.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s num_bases <input_fastq_file>\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            sys.stderr.write(
                "Removes any non-alnum character from the sequences' header and replaces it with '_'.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<out_filtered_fastq_file>]\n"
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            sys.stderr.write(
                "Removes any non-alnum character from the sequences' header and replaces it with '_'. Also, removes all reads larger than 50kbp.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<out_filtered_fastq_file>]\n"
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            sys.stderr.write(
                "Wraps FASTA/FASTQ files to have line length up to a given number of characters.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s line_length <input_fastq_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 4:
            sys.stderr.write(
                "Converts lowercase bases (e.g. masking regions) to upper case.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<out_filtered_fastq_file>]\n"
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 4:
            sys.stderr.write(
                "Replace headers of FASTQ sequences with their ordinal numbers.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<out_filtered_fastq_file>]\n"
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 4:
            sys.stderr.write(
                "If a FASTQ file contains whitespaces, remove everything after the first whitespace.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<out_filtered_fastq_file>]\n"
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 4:
            sys.stderr.write(
                "Makes all headers in a FASTQ file unique by adding a number at the end of the header. Also, splits headers up to the first whitespace.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<out_filtered_fastq_file>]\n"
        ...
        if len(sys.argv) < 5 or len(sys.argv) > 6:
            sys.stderr.write(
                "Output only reads which have average base qualities above or below certain threshold.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s lte_gte threshold <input_fastq_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            sys.stderr.write(
                '\tlte_gte - Select which reads to output. Can be either "lt", "lte", "gt", "gte" or "eq".\n'
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if (lte_gte in ["lt", "lte", "gt", "gte", "eq"]) == False:
            sys.stderr.write(
                'ERROR: Incorrect value of the lte_gte parameter. Should be either "<" or ">".'
            )
            exit(1)
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            sys.stderr.write(
                "Tool for obtaining basic stats from FASTA/FASTQ files, such as number of sequences, total sequence length, average sequence length, etc.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> [<input_reference_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            sys.stderr.write(
                "\t<input_reference_file> - If a reference is given, coverage is caluclated using the Lander-Waterman equation.\n"
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) == 4:
            ...
            [
                ...
                ref_average_seq_len,
                max_seq_len,
            ] = fastqparser.count_seq_length(reference_path)
        ...
        [
            ...
            average_seq_len,
            max_seq_len,
        ] = fastqparser.count_seq_length(input_fastq_path)
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 3:
            sys.stderr.write(
                "This is not an actual filter, but counts the number of 1d or 2d reads.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file>\n"
        ...
        if len(sys.argv) < 5 or len(sys.argv) > 5:
            sys.stderr.write(
                "Subsamples a given fasta/fastq file to a given coverage. Prints the result to stdout.\n"
            )
            sys.stderr.write(
                "If the initail coverage is too low, prints out the original file.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fasta(q)_file> <desired_coverage> <ref_genome_size>\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                "Outputs all fasta/fastq sequences for an input file, whose headers are present in a given target fasta/fastq file.\n"
            )
            sys.stderr.write("Prints results to stdout.\n")
            sys.stderr.write(
                "Useful when subsampling paired end reads in separate files.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fasta(q)_file> <target_fasta(q)_file>\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 3:
            sys.stderr.write(
                'Outputs a given fastq file in fasta format. Replaces the ":" characters with blank spaces.\n'
            )
            sys.stderr.write(
                "Additionally, replaces colons in header with spaces (relevant for running L&S pipeline).\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file>\n"
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 3:
            sys.stderr.write(
                'Outputs a given fastq file in fasta format. Does not replace the ":" characters with blank spaces (unlike fastq2fasta).\n'
            )
            sys.stderr.write(
                "Additionally, replaces colons in header with spaces (relevant for running L&S pipeline).\n"
            )
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 4:
            sys.stderr.write(
                "This is not an actual filter, but removes extra spaces from nanopore paths.\n"
            )
            sys.stderr.write("So that reads file can be used with nanopolish.\n")
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> <fast5_root_path>\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 3:
            sys.stderr.write(
                "Outputs the distribution of reads in a fasta/fastq file.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_file>\n" % (os.path.basename(sys.argv[0]), sys.argv[1])
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            sys.stderr.write(
                'Extracts only reads with typical %s (nanopore) headers (those containing either "1d", "template" or "complement"; "2d" or "twodir" in their header).\n'
                % (sys.argv[1])
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
        ...
        if sys.argv[1] == "1d":
            ...
            filter_seqs_by_header_list(
                input_fastq_path, ["2d", "twodir"], out_fastq_path, fp_out
            )
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            sys.stderr.write(
                "Takes a FASTA/FASTQ file and a file containing seq IDs (0-offset). Extracts all seqs with given IDs.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_id_file> <input_fastq_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) < 5 or len(sys.argv) > 6:
            sys.stderr.write(
                "Extracts bases from all sequences in a FASTA file between specified coordinates. End coordinate is not inclusive.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> start end [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        extract_subseqs(
            input_fastq_path, start_coord, end_coord, out_fastq_path, fp_out
        )
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            sys.stderr.write(
                "Takes a multifasta file of multiple sequence alignments, and produces a majority vote consensus. Only [ACTG] bases are considered for output. Gaps in the input file are denoted by either a '.' or a '-'.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            sys.stderr.write(
                "Separate a FASTA/FASTQ file into individual sequence files. File names will be numbers assigned to sequences sequentially.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> <out_folder> [headers_as_fn]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            sys.stderr.write(
                '  headers_as_fn - if "true", output files will be named the same as the header of the corresponding sequence.\n'
            )
            sys.stderr.write("\n")
            exit(0)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 4:
            ...
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_fastq_file> <out_converted_fastq_file>\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            sys.stderr.write(
                "Takes an input FASTA/FASTQ file and converts it to a FASTQ format if necessary, while replacing all QVs with a given qv_val value.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s qv_val <input_fastq_file> [<out_filtered_fastq_file>]\n"
                % (os.path.basename(sys.argv[0]), sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(0)
