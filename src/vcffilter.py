...
def extract_region(vcf_file, chrname, start_pos, end_pos):
    ...
    try:
        ...
        sys.stderr.write(
            '[%s] ERROR: Could not open file "%s" for reading!' % (__name__, vcf_file)
        )
        exit(1)
    ...
    for line in fp_in:
        ...
        if split_line[0] == chrname:
            if pos_position < 0:
                sys.stderr.write(
                    "ERROR: Could not find parameter description line, needed to determine the POS parameter!\n"
                )
                exit(1)
...
def split_multiallelic_snps(vcf_file):
    ...
    for line in fp_in:
        ...
        if alt_position < 0:
            sys.stderr.write(
                "ERROR: Could not find parameter description line, needed to determine the ALT parameter!\n"
            )
            exit(1)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "region":
        if len(sys.argv) != 6:
            sys.stderr.write(
                "Extracts only variant lines which fall within a defined region. Output is on stdout. Comment lines are output as well.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_vcf_file> chrname startpos endpos\n"
