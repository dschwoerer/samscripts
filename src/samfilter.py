...
def filter_wrong_cigars(sam_file, out_filtered_sam_file):
    ...
    try:
        ...
        sys.stderr.write(
            '[%s] ERROR: Could not open file "%s" for reading!\n' % (__name__, sam_file)
        )
        exit(1)
    ...
    for line in fp_in:
        sys.stderr.write(
            "\rLine %d, num_accepted: %d, num_rejected: %d"
            % (i, num_accepted, num_rejected)
        )
        ...
        if sam_line.IsAlignmentSane() == True:
            ...
            sys.stderr.write(
                "\tRejected: line: %d, qname: %s len: %d, len_from_cigar: %d\n"
                % (
                    i,
                    sam_line.qname,
                    len(sam_line.seq),
                    sam_line.CalcReadLengthFromCigar(),
                )
            )
    ...
    sys.stderr.write("\n")
    sys.stderr.write(
        "Number of alignments with equal seq length and CIGAR length: %d (%.2f%%)\n"
        % (
            num_accepted,
            (float(num_accepted) / float(num_accepted + num_rejected)) * 100.0,
        )
    )
    sys.stderr.write(
        "Number of faulty alignments: %d (%.2f%%)\n"
        % (
            num_rejected,
            (float(num_rejected) / float(num_accepted + num_rejected)) * 100.0,
        )
    )
...
def filter_large_cigar_ops(sam_file, max_cigar_ops, out_filtered_sam_file):
    ...
    for line in fp_in:
        ...
        if num_cigar_ops <= max_cigar_ops:
            ...
            sys.stderr.write(
                "\tRejected: line: %d, qname: %s, len: %d, num_cigar_ops: %d, max_cigar_ops: %d\n"
    ...
    sys.stderr.write("\n")
    sys.stderr.write(
        "Number of good alignments: %d (%.2f%%)\n"
        % (
            num_accepted,
            (float(num_accepted) / float(num_accepted + num_rejected)) * 100.0,
        )
...
def filter_sam_by_mapq(sam_file, mapq_limit, out_filtered_sam_file):
    ...
    try:
        ...
        sys.stderr.write(
            '[%s] ERROR: Could not open file "%s" for reading!' % (__name__, sam_file)
        )
        exit(1)
...
def filter_sam_by_length(sam_file, threshold, out_filtered_sam_file):
    ...
    for line in fp_in:
        ...
        if (
            len(sam_line.seq) + sam_line.clip_count_front + sam_line.clip_count_back
        ) >= threshold:
            fp_out.write(line)
            num_accepted += 1
        else:
...
def filter_sam_by_evalue(sam_file, threshold, out_filtered_sam_file):
    ...
    sys.stderr.write(
        ...
        % (
            num_unmapped,
            (float(num_unmapped) / float(num_accepted + num_rejected)) * 100.0,
        )
    )
...
def filter_qnames(
    ...
    pattern_file,
    include_or_exclude,
    sam_parameter_to_compare,
    out_filtered_sam_file,
):
    fp_in = None
    fp_patterns = None
    fp_out = None
    ...
    try:
        ...
        sys.stderr.write(
            '[%s] ERROR: Could not open file "%s" for reading!'
            % (__name__, pattern_file)
        )
        exit(1)

    patterns = [
        line.strip() for line in fp_patterns.readlines() if (len(line.strip()) > 0)
    ]
    fp_patterns.close()
    ...
    for line in fp_in:
        ...
        if include_or_exclude == "include":
            ...
            for pattern in patterns:
                if (
                    sam_parameter_to_compare == "qname"
                    and pattern.lower() in sam_line.qname.lower()
                ) or (
                    sam_parameter_to_compare == "rname"
                    and pattern.lower() in sam_line.rname.lower()
                ):
            ...
            for pattern in patterns:
                if (
                    sam_parameter_to_compare == "nqname"
                    and pattern.lower() in sam_line.qname.lower()
                ) or (
                    sam_parameter_to_compare == "nrname"
                    and pattern.lower() in sam_line.rname.lower()
                ):
                    is_current_accepted = True
                    break
...
def filter_uniqe_best(sam_file, out_filtered_sam_file, out_rejected_file=None):
    ...
    [hashed_sam, num_sam_lines, num_unique_sam_lines] = utility_sam.HashSAMWithFilter(
        sam_file, {}
    )
...
def filter_mapped(sam_file, mapped, out_filtered_sam_file):
    ...
    for line in fp_in:
        ...
        if (mapped == 0 and sam_line.IsMapped() == False) or (
            mapped != 0 and sam_line.IsMapped() == True
        ):
            fp_out.write(line)
            num_accepted += 1
        else:
...
def add_dummy_quality_values_nanopore(
    phred_qv_1d, phred_qv_2d, sam_file, out_filtered_sam_file
):
    fp_in = None
    fp_out = None
...
def extract_region_full(region, sam_file, out_filtered_sam_file):
    try:
        ...
        sys.stderr.write(
            'ERROR: Region not formatted correctly! Given region: "%s", correct format: "chromosome:start-end.\n',
            region,
        )
        exit(1)
    ...
    for line in fp_in:
        ...
        if (
            sam_line.IsMapped() == True
            and sam_line.rname.startswith(chromosome) == True
            and sam_line.pos <= start
            and (sam_line.pos + sam_line.CalcReferenceLengthFromCigar()) >= end
        ):
            fp_out.write(line)
            num_accepted += 1
        else:
...
def extract_region_partial(region, sam_file, out_filtered_sam_file):
    ...
    for line in fp_in:
        ...
        if (
            sam_line.IsMapped() == False
            or sam_line.rname.startswith(chromosome) == False
            or (alignment_start < start and alignment_end < start)
            or (alignment_start > end and alignment_end > end)
        ):
            num_rejected += 1
        else:
            fp_out.write(line)
...
def filter_1d_2d(is_1d_or_2d, sam_file, out_filtered_sam_file):
    ...
    for line in fp_in:
        ...
        if (
            ...
            and (
                (
                    ("twodir" in qname) == False
                    and ("-2d" in qname) == False
                    and ("_2d" in qname) == False
                )
                or ("-1d" in qname) == True
                or ("_1d" in qname) == True
            )
        ) or (
            is_1d_or_2d == "2d"
            and (
                ("twodir" in qname) == True
                or ("-2d" in qname) == True
                or ("_2d" in qname) == True
            )
        ):
...
def extract_features_csv(sam_file, out_csv_file):
    ...
    for line in fp_in:
        ...
        feature_line.append(
            "1" if ("escherichia coli" in sam_line.rname.lower()) else "0"
        )
...
def filter_out_of_bounds(sam_file, out_filtered_sam_file, out_rejected_sam_file=None):
    ...
    try:
        ...
        sys.stderr.write(
            '[%s] ERROR: Could not open file "%s" for writing!'
            % (__name__, out_rejected_sam_file)
        )
        exit(1)
    ...
    for line in fp_in:
        ...
        if sam_line.IsMapped() == False or (
            sam_line.IsMapped() == True
            and pos_start >= 0
            and pos_end < reference_length
        ):
            fp_out.write(line)
            num_accepted += 1
        else:
...
def sam_stats(sam_file, reads_fastq=""):
    ...
    sys.stdout.write(
        "Unique mapped lines: %d / %d (%.2f%%)\n"
        % (
            num_mapped_unique,
            num_qnames_in_sam_file,
            float(num_mapped_unique) / float(num_qnames_in_sam_file) * 100.0,
        )
    )
    if reads_fastq != "":
        sys.stdout.write(
            ...
            % (
                num_mapped_unique,
                len(read_qnames),
                float(num_mapped_unique) / float(len(read_qnames)) * 100.0,
            )
        )
...
def generate_AS(
    ...
    for line in fp_in:
        ...
        if (
            len(line) == 0
            or line[0] == "@"
            or (("AS:i:" in line) and ("NM:i:" in line))
        ):
            # if (len(line) == 0 or line[0] == '@'):
            fp_out.write(line + "\n")
            continue
        ...
        try:
            ...
            sys.stderr.write(
                'Warning: Could not find reference "%s"! Skipping alignment.\n'
                % (rname)
            )
            fp_out.write(line + "\n")
            num_rejected += 1
        ...
        cigar_pos_list = sam_line.CalcCigarStartingPositions(
            separate_matches_in_individual_bases=True
        )
        for cigar_pos_info in cigar_pos_list:
            [cigar_count, cigar_op, pos_on_reference, pos_on_read] = cigar_pos_info
            if cigar_op == "M":
                if (
                    sam_line.seq[pos_on_read - clip_start]
                    == ref_seqs[ref_id][pos_on_reference]
                ):
                    AS += match_score
                else:
                    AS += mismatch_penalty
            ...
            if line.startswith("@SQ"):
                ...
                for param in split_line:
                    if param.startswith("SN:"):
                        ...
                        sys.stderr.write(
                            'Found hname: "%s", replacing with: "%s".\n'
                            % (hname, new_hname)
                        )
...
def plot_alignments(ref_file, sam_file):
    ...
    for line in fp_in:
        ...
        fp_out.write(
            "Aligned pos: %d (1-based) or %d (0-based)\n" % (sam_line.pos, pos)
        )
        ...
        for [
            ...
            pos_on_ref,
            pos_on_read,
        ] in sam_line.CalcCigarStartingPositions(split_cigar_in_basic_format=False):
...
def sam_info(sam_file, reads_file=None):
    ...
    try:
        fp_out.write(
            "Percent of mapped reads which were 1d: %.2f%%\n"
            % (float(num_mapped_1d) / float(num_mapped_1d + num_mapped_2d) * 100.0)
        )
        fp_out.write(
            "Percent of mapped reads which were 2d: %.2f%%\n"
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "mapq":
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Extract SAM lines which have mapping quality higher or equal to a given threshold.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s mapq_limit <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Extract SAM lines which have alignment score (AS field) higher or equal to a given threshold.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s as_limit <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Extract SAM lines which have sequence length higher or equal to a given threshold.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s len_limit <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                "Takes an input SAM file and for each entry compares the SEQ length and the length obtained from counting the CIGAR operations. These should match. If they do not match, these lines are filtered out. The output SAM contains only lines with matching lengths.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Takes in a SAM file and a file containing qname patterns (one pattern in one line). Outputs all sam lines containing a pattern.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_patterns_file> <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if sam_file == out_filtered_sam_file:
            ...
            exit(0)
        filter_qnames(
            sam_file, patterns_file, "include", sys.argv[1], out_filtered_sam_file
        )
        exit(0)
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Takes in a SAM file and a file containing qname patterns (one pattern in one line). Outputs all sam lines *not* containing a pattern.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_patterns_file> <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if sam_file == out_filtered_sam_file:
            ...
            exit(0)
        filter_qnames(
            sam_file, patterns_file, "exclude", sys.argv[1], out_filtered_sam_file
        )
        exit(0)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            sys.stderr.write(
                "Filteres only unique qname lines. If there are multiple alignments with same qname, then only the one with highest mapq/alignment score will be output.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if len(sys.argv) == 5:
            ...
            if sam_file == out_rejected_file:
                sys.stderr.write(
                    "ERROR: Output for rejected SAM lines and input files are the same!\n"
                )
                exit(0)
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                "Filteres only unique qname lines. If there are multiple alignments with same qname, the ones with AS <= 0 are discarded, and then only the one with highest mapq/alignment score will be output.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Output only mapped (or unmapped) alignments. If parameter is 0, unmapped will be output, otherwise only mapped.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s mapped <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            sys.stderr.write(
                "Output only qnames of mapped (or unmapped) alignments. If parameter is 0, unmapped will be output, otherwise only mapped.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s mapped <input_sam_file> [<out_filtered_sam_file>]\n"
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                'Extracts only 1d reads from the given SAM file. If the qname does not contain keywords "twodir" or "-2d" or contains "-1d", it is considered 1d.\n'
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                'Extracts only 2d reads from the given SAM file. If the qname contains keywords "twodir" or "-2d" it is considered as 2d.\n'
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                "Extract various features from the SAM file alignments, and create a CSV formatted output suitable for analysis with RapidMiner.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Adds base quality values to the SAM lines from the given reads FASTQ file.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <reads_fastq> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) != 5:
            ...
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s phredqv <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            sys.stderr.write("\n")
            sys.stderr.write(
                "\t\tphredqv - decimal value of the Phred substitution score\n"
            )
            exit(0)
        ...
        if len(sys.argv) != 6:
            ...
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s phredqv_default phredqv_1d phredqv_2d <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            sys.stderr.write("\n")
            sys.stderr.write(
                "\t\tphredqv_1d - average decimal value of the Phred substitution score for 1d reads [18%% is 7.447 Phred]\n"
            )
            sys.stderr.write(
                "\t\tphredqv_2d - average decimal value of the Phred substitution score for 2d reads [11%% is 9.586 Phred]\n"
            )
            exit(0)
        ...
        if sam_file == out_filtered_sam_file:
            sys.stderr.write("ERROR: Output and input files are the same!\n")
            exit(0)
        add_dummy_quality_values_nanopore(
            phred_qv_1d, phred_qv_2d, sam_file, out_filtered_sam_file
        )
        exit(0)
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Extracts only alignments that are fully overlapping given genomic region (in SAMtools format, i.e. chrom_name:start-end).\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s region <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Extract SAM lines which have E-value less than a given threshold (custom field ZE in SAM output).\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s threshold <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) != 3:
            sys.stderr.write(
                "Calculates some simple statistics on a SAM file, such as number of unique alignment, number of mapped alignments, and similar.\n"
            )
            sys.stderr.write("Usage:\n")
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Check if alignment start and end positions fall within the boundaries of the reference genome.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file> <out_rejected_sam_file>\n"
        ...
        if (
            sam_file == out_filtered_sam_file
            or sam_file == out_rejected_sam_file
            or out_filtered_sam_file == out_rejected_sam_file
        ):
            sys.stderr.write("ERROR: Output and input files are the same!\n")
            exit(0)
        ...
        if len(sys.argv) != 5:
            sys.stderr.write(
                "Calculates a simple alignment score if not previously present, using a model [1, -1, -1, -1].\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <reference_file> <input_sam_file> <out_filtered_sam_file>\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                "Changes the qnames and the rnames of alignments not to include special characters.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            ...
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file> [<reference_file>]\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                "BAM file has a length limit on the number of CIGAR operations that it can store. The longer, erroneous reads might exceed these limits. Concretely, BAM format specification states:\n"
            )
            sys.stderr.write(
                "\tflag nc\tFLAG<<16|n cigar op;19 n cigar op is the number of operations\tuint32 t\n"
            )
            sys.stderr.write(
                "This means that the BAM file can hold up to 2^16-1 CIGAR operations.\n\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> <out_filtered_sam_file>\n"
        ...
        if len(sys.argv) < 3 or len(sys.argv) > 4:
            ...
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s %s <input_sam_file> [<reads_fastq_path>]\n"
                % (sys.argv[0], sys.argv[1])
            )
            exit(0)
