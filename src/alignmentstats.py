...
def print_usage_and_exit():
    print("Usage:")
    print(
        "\t%s file|folder mode_code <sam_path> <reference_path> <reads_path> [consensus_coverage_threshold] [<simulated_sam_path>]"
        % sys.argv[0]
    )
    print("")
...
def run_from_args(cmd_args):
    ...
    if mode == "calc":
        mode_code = (
            MODE_CODE_CALC_CONSENSUS
            | MODE_CODE_CALC_ERROR_RATE
            | MODE_CODE_CALC_ERROR_RATE_INDELS_AS_EVENTS
            | MODE_CODE_CALC_READ_COUNTS
        )
    elif mode == "hcalc":
        mode_code = (
            MODE_CODE_CALC_CONSENSUS
        ...
        mode_code = (
            MODE_CODE_COLLECT_CONSENSUS
            | MODE_CODE_COLLECT_ERROR_RATE
            | MODE_CODE_COLLECT_ERROR_RATE_INDELS_AS_EVENTS
            | MODE_CODE_COLLECT_READ_COUNTS
        )
    elif mode == "hcollect":
        mode_code = (
            MODE_CODE_COLLECT_CONSENSUS
    ...
    if file_or_folder == "file":
        ...
        sys.stdout.write(
            analyze_single_sam(
                ...
                reference_file,
                reads_path,
                simulated_sam_path=simulated_sam_path,
        ...
        sys.stderr.write(
            "Warning: Analysis of all SAMs in a given folder not implemented yet! Skipping.\n"
        )
        pass
    else:
...
def ParseMemTime(sam_file):
    ...
    try:
        ...
        sys.stderr.write(
            'ERROR: Could not parse memtime stats from "%s"!\n' % memtime_path
        )
...
def ParseVariantStats(variant_summary_path):
    ...
    try:
        ...
        sys.stderr.write(
            'ERROR: Could not parse variant stats from "%s"!\n' % variant_summary_path
        )
