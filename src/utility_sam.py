...
class SAMLine:
    ...
    def VerboseFormatLine(self):
        ...
        line = (
            "qname = %s\tactual_ref_pos = %d\tmapped_pos_with_shift = %d\tmin_distance = %d\tmapq = %d\tactual_ref_reverse = %d\treverse = %d\tis_correct_ref_and_orient = %d\tis_header_deformed = %d\tflag = %d\trname = %s\tpos = %d\tis_duplicate = %d\tis_filtered_out = %d\tis_best_of_duplicates = %d\tsecondary = %s\tpaired = %s\t"
            % (
                self.qname,
                self.actual_ref_pos,
                self.mapped_pos_with_shift,
                self.min_distance,
                self.mapq,
                self.actual_ref_reverse,
                self.IsReverse(),
                int(self.is_correct_ref_and_orient),
                self.is_header_deformed,
                self.flag,
                self.rname,
                self.pos,
                int(self.is_duplicate),
                int(self.is_filtered_out),
                int(self.is_best_of_duplicates),
                str(self.IsSecondary()),
                self.IsPaired(),
            )
        )
    ...
    def ParseLine(self, line, sam_basename=""):
        ...
        while i < len(split_line):
            ...
            if len(split_optional) < 2:
                i += 1
                continue
            self.optional[split_optional[0].strip()] = split_optional[-1].strip()
            # Example of an optional parameter: AS:i:717 .
            i += 1

        self.alignment_score = (
            -1 if (("AS" in self.optional) == False) else int(self.optional["AS"])
        )
        self.edit_distance = (
            -1 if (("NM" in self.optional) == False) else int(self.optional["NM"])
        )
        self.fp_filter = (
            -1.0 if (("X5" in self.optional) == False) else float(self.optional["X5"])
        )
        if self.fp_filter == -1.0:
            self.fp_filter = (
                -1.0
                if (("ZF" in self.optional) == False)
                else float(self.optional["ZF"])
            )
        ...
        if "X3" in self.optional:
            ...
            # print params;
            # print '';
            m = re.search(r"_supp\[(.*?)\]", params)
            self.optional_graphmap["supp"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_AS\[(.*?)\]", params)
            self.optional_graphmap["AS"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_AS_std\[(.*?)\]", params)
            self.optional_graphmap["AS_std"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_AS_cov_bases\[(.*?)\]", params)
            self.optional_graphmap["AS_cov_bases"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_AS_read_len\[(.*?)\]", params)
            self.optional_graphmap["AS_read_len"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_AS_query_len\[(.*?)\]", params)
            self.optional_graphmap["AS_query_len"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_num_kmers=(.*?)_", params)
            self.optional_graphmap["num_kmers"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_cov_bases=(.*?)_", params)
            self.optional_graphmap["cov_bases"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"lcs_length=(.*?)_", params)
            self.optional_graphmap["lcs_length"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_match_rate=(.*?)_", params)
            self.optional_graphmap["match_rate"] = (
                float(m.group(1)) if (m and m.groups) else 1.0
            )
            m = re.search(r"_mismatch_rate=(.*?)\t", params)
            self.optional_graphmap["mismatch_rate"] = (
                float(m.group(1)) if (m and m.groups) else 0.0
            )
            m = re.search(r"_num_eq_ops=(.*?)_", params)
            self.optional_graphmap["num_eq_ops"] = (
                float(m.group(1)) if (m and m.groups) else 0.0
            )
            m = re.search(r"_num_x_ops=(.*?)_", params)
            self.optional_graphmap["num_x_ops"] = (
                float(m.group(1)) if (m and m.groups) else 0.0
            )
            m = re.search(r"_num_i_ops=(.*?)_", params)
            self.optional_graphmap["num_i_ops"] = (
                float(m.group(1)) if (m and m.groups) else 0.0
            )
            m = re.search(r"_num_d_ops=(.*?)_", params)
            self.optional_graphmap["num_d_ops"] = (
                float(m.group(1)) if (m and m.groups) else 0.0
            )
        ...
        self.actual_ref_pos = 0
        self.mapped_pos_with_shift = 0
        # This is assigned only after evaluation of the mapping position. It is equal to self.clipped_pos if self.qname is found in the reference SAM file, otherwise it is equal to 0.
        self.min_distance = -1

        self.clipped_pos = self.pos
        # The position of the read subtracted by the amount of clipped bases at the begining of the read.
        self.clip_op_front = ""
        self.clip_count_front = 0
        self.clip_op_back = ""
        self.clip_count_back = 0
    ...
    def FormatAccuracy(self):
        ...
        line += "reverse_hit = %s\t" % (
            str(self.actual_ref_reverse == self.IsReverse())
        )
        ...
        line += "AS = %d\t" % (
            0 if (("AS" in self.optional) == False) else int(self.optional["AS"])
        )
        ...
        line += "len = %d\t" % (
            len(self.seq)
            - (self.clip_count_front if (self.clip_op_front == "S") else 0)
            - (self.clip_count_back if (self.clip_op_back == "S") else 0)
        )
    ...
    def CalcExtendedCIGAR(self, reference):
        ...
        refpos = (
            self.pos - 1
        )  # start of alignment within referece, clipped bases are skipped
        ...
        for op in operations:
            ...
            if opsize <= 0:
                continue
            if optype in "M=":
                mcigar = getExtendedCIGAR(
                    reference[refpos : refpos + opsize],
                    self.seq[seqpos : seqpos + opsize],
                )
                extcigar += mcigar
                refpos += opsize
                seqpos += opsize
        ...
        while i < len(cigar_pos_list):
            ...
            if cigar_op in "M=X":
                if pos_on_read == base_position_in_read:
                    ...
                    pos_on_read < base_position_in_read
                    and (pos_on_read + cigar_count) > base_position_in_read
                ):
        ...
        while i < len(cigar_pos_list):
            ...
            if cigar_op in "M=XD":
                if pos_on_reference == base_position_in_reference:
                    ...
                    pos_on_reference < base_position_in_reference
                    and (pos_on_reference + cigar_count) > base_position_in_reference
                ):
        ...
        for i in range(0, len(cigar_pos_list)):
            ...
            if cigar_op in "M=XI":
                if pos_on_read == start_pos:
                    ...
                    pos_on_read < start_pos and (pos_on_read + cigar_count) > start_pos
                ):
                    start_event = i
                    break
    ...
    def CalcCigarStartingPositions(
        self,
        separate_matches_in_individual_bases=False,
        separate_indels_in_individual_bases=False,
        switch_ins_and_dels=False,
        split_cigar_in_basic_format=True,
    ):
            ...
            if (cigar_op in "SH") == True:
                ...
                if separate_matches_in_individual_bases == False:
                    ...
                    for j in range(0, cigar_count):
                        cigar_pos_list.append(
                            [1, cigar_op, pos_on_reference + 0, pos_on_read + 0]
                        )
                        pos_on_reference += 1
                        pos_on_read += 1
    ...
    def IsAlignmentSane(self):
        ...
        if seq_len != alignment_length_on_seq:
            sys.stderr.write(
                "(III) Insane because: (seq_len != alignment_length_on_seq)\n"
            )
            return False
    ...
    def CountErroneousWindows(self, window_len, min_error_rate):
        ...
        cigars = self.CalcCigarStartingPositions(
            ...
            if cigar_op in "SIM=X":
                ...
                while i < cigar_count:
                    try:
                        ...
                        print(cigar)
                        print(
                            seq_len, pos_on_read, num_hardclipped_bases, i, len(cigars)
                        )
                    ...
                    if cigar_op == "I":
                        real_bases_insertions[
                            pos_on_read + i - num_hardclipped_bases
                        ] = 1
                    i += 1
            elif cigar_op in "D":
        ...
        err_window_rate = (
            (float(num_over_threshold) / float(num_windows))
            if (num_windows > 0)
            else (-1.0)
        )
...
def possible_split_alignment(samline1, samline2, threshold=0):
    ...
    if threshold > 0:
        if (rpos1 > rpos2 and rpos1 - rpos2 > threshold) or (
            rpos2 > rpos1 and rpos2 - rpos1 > threshold
        ):
            return False
    ...
    for op1 in operations1:
        if op1[1] in (
            ...
            "=",
            "X",
        ):  # NOTE: I'm only interested in intersects in alignment matches!
            end1 = pos1 + int(op1[0])
            ...
            for op2 in operations2:
                if op2[1] in ("M", "=", "X"):
                    ...
                    if not (end2 < pos1 or pos2 > end1):
                        alignment_intersect = (
                            True  # NOTE: Here, a return could be called right away!
                        )
                        return False
...
def getExtendedCIGAR(string1, string2, countMutations=True):
    if len(string1) != len(string2):
        raise Exception(
            "Cannot calculate extended CIGAR for strings of different length!"
        )
    ...
    for sam_line in sam_lines:
        ...
        if qname in ret:
            ...
            for existing_sam in current_hash:
                if (sam_line.IsSecondary() == True) or (
                    sam_line.IsSecondary() == existing_sam.IsSecondary()
                    and sam_line.IsReverse() == existing_sam.IsReverse()
                ):
            ...
            if sam_line.IsSecondary() == False:
                num_unique_lines += 1
                # Count only unique sequences, but not secondary ones.
...
def GetBasicStats(sam_lines, allowed_distance=1):
    ...
    for sam_line in sam_lines:
        if sam_line.IsMapped() and sam_line.is_duplicate == 0:
            if (
                sam_line.is_correct_ref_and_orient == 1
                and sam_line.min_distance <= allowed_distance
            ):
                true_positive += 1
            else:
...
def GetDistanceHistogramStats(sam_lines, distance_limits):
    sorted_lines_by_distance = sorted(
        sam_lines, key=lambda sam_line: sam_line.min_distance
    )
    ...
    while i < len(sorted_lines_by_distance):
        ...
        if sam_line.IsMapped() and sam_line.is_duplicate == 0:
            if sam_line.is_correct_ref_and_orient == 1:
                if (
                    sam_line.min_distance
                    < sorted_distance_limits[current_distance_index]
                ):
                    ...
                    continue
                    # Do not increase i, because we want to check the current SAM line also.
    ...
    while i < len(sorted_distance_limits):
        ret_distance_counts[i] = total_correct
        # ret_distance_counts[current_distance_index];
        i += 1
...
def GetDistanceHistogramStatsScaleDuplicates(
    sam_lines, distance_limits, scale_by_num_occurances=True
):
    sorted_lines_by_distance = sorted(
        sam_lines, key=lambda sam_line: sam_line.min_distance
    )
...
def GetMapqHistogramStats(sam_lines, mapq_limits, allowed_distance):
    sorted_lines_by_mapq = sorted(
        sam_lines, reverse=True, key=lambda sam_line: sam_line.mapq
    )
...
def CountMappedReads(sam_file):
    ...
    sam_mapped_reads = {}
    num_mapped_bases = 0
    # Count of the number of non-clipped bases in an alignment. Only highest-scoring alignments are considered for each read,
    total_num_bases = 0
    highest_scoring_alignment_for_read = {}
    # Hash to keep track of the highest scoring alignment for a read.
    ...
    for line in fp:
        ...
        if sam_line.IsMapped() == True:
            ...
            try:
                if (
                    highest_scoring_alignment_for_read[sam_line.qname].chosen_quality
                    < sam_line.chosen_quality
                ):
...
def CompareBasePositions(query_sam, ref_sam, switch_ins_and_dels=False):
    ...
    num_mapped_bases = (
        len(qsam_ref_coords) - query_sam.clip_count_front - query_sam.clip_count_back
    )
    num_ref_bases = (
        len(rsam_ref_coords) - ref_sam.clip_count_front - ref_sam.clip_count_back
    )

    if (
        len(qsam_ref_coords) < len(rsam_ref_coords)
        or query_sam.IsMapped() == False
        or ref_sam.IsMapped() == False
        or query_sam.rname != ref_sam.rname
        or query_sam.IsReverse() != ref_sam.IsReverse()
    ):
    ...
    for cigpos in query_cigpos:
        ...
        if cig_op in "M=X":
            qsam_ref_coords[pos_on_query : (pos_on_query + cig_count)] = list(
                range(pos_on_ref, (pos_on_ref + cig_count))
            )
        elif cig_op == "I":
            qsam_ref_coords[pos_on_query : (pos_on_query + cig_count)] = [
                -1
            ] * cig_count
        elif cig_op == "S":
            qsam_ref_coords[pos_on_query : (pos_on_query + cig_count)] = [
    ...
    for cigpos in ref_cigpos:
        [cig_count, cig_op, pos_on_ref, pos_on_query] = cigpos
        if cig_op in "M=X":
            rsam_ref_coords[pos_on_query : (pos_on_query + cig_count)] = list(
                range(pos_on_ref, (pos_on_ref + cig_count))
            )
        elif (switch_ins_and_dels == False and cig_op == "I") or (
            switch_ins_and_dels == True and cig_op == "D"
        ):
            rsam_ref_coords[pos_on_query : (pos_on_query + cig_count)] = [
                -1
            ] * cig_count
        elif cig_op == "S":
            rsam_ref_coords[pos_on_query : (pos_on_query + cig_count)] = [
...
def CountCorrectlyMappedBases(
    hashed_sam_lines,
    hashed_reference_sam,
    out_summary_prefix="",
    sam_basename="",
    switch_ins_and_dels=False,
):
    ...
    if out_summary_prefix != "":
        try:
            ...
            exit(1)
    sys.stderr.write(
        "Starting to count the number of correctly mapped bases in the tested SAM file!\n"
    )
    ...
    for qname in list(hashed_sam_lines.keys()):
        ...
        if "blasr" in sam_basename.lower():
            ...
            if sam_line.clip_count_front != 0 or sam_line.clip_count_back != 0:
                sys.stderr.write(
                    'BLASR CIGAR contains clipping! Please revise clipped_pos! Read: "%s".\n'
                    % sam_line.qname
                )
        ...
        if (qname in hashed_reference_sam) == False:
            sys.stderr.write(
                '\tERROR: Reference SAM does not contain qname "%s"!\n' % (qname)
            )
            continue

        sam_reference = hashed_reference_sam[qname][0]

        [num_correct_bases, num_mapped_bases, num_ref_bases] = CompareBasePositions(
            sam_line, sam_reference, switch_ins_and_dels
        )
    ...
    precision = (
        (100.0 * float(sum_correct_bases) / float(sum_mapped_bases))
        if (sum_mapped_bases > 0)
        else 0.0
    )
    recall = (
        (100.0 * float(sum_correct_bases) / float(total_ref_bases))
        if (total_ref_bases)
        else 0.0
    )
