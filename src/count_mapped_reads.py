...
def CountMappedReads(sam_file, out_stats_prefix="", num_reads_in_reference_sam=1):
    ...
    ret_lines += (
        "Number of alignments in SAM file which are marked mapped: %d (%.2f%%)\n"
        % (
    ...
    ret_lines += "Number of reads in SAM file: %d (%.2f%%)\n" % (
        len(list(sam_reads.keys())),
        (float(len(list(sam_reads.keys()))) / float(num_alignments)) * 100.0,
    )
    ret_lines += (
        "Number of mapped reads in SAM file: %d (percent_reads: %.2f%%, percent_alignments: %.2f%%)\n"
        % (
            len(list(sam_mapped_reads.keys())),
            (
                float(
                    len(list(sam_mapped_reads.keys()))
                    / float(len(list(sam_reads.keys())))
                )
                * 100.0
            ),
            (float(len(list(sam_mapped_reads.keys()))) / float(num_alignments)) * 100.0,
        )
    )
    ret_lines += (
        "Number of unique reads in SAM file: %d (percent_reads: %.2f%%, percent_alignments: %.2f%%)\n"
        % (
            num_unique_reads,
            (float(num_unique_reads) / float(len(list(sam_reads.keys())))) * 100.0,
            (float(num_unique_reads) / float(num_alignments)) * 100.0,
        )
    )
    ret_lines += (
        "Number of reads with multiple alignments: %d (percent_reads: %.2f%%, percent_alignments: %.2f%%)\n"
        % (
            (len(list(sam_reads.keys())) - num_unique_reads),
            (
                float((len(list(sam_reads.keys())) - num_unique_reads))
                / float(len(list(sam_reads.keys())))
            )
            * 100.0,
            (
    ...
    ret_lines += "Percent of mapped reads compared to the reference SAM: %.2f%%\n" % (
        (float(len(list(sam_mapped_reads.keys()))) / float(num_reads_in_reference_sam))
        * 100.0
    )
    ret_lines += (
        "Percent of reads with unique alignments compared to the reference SAM: %.2f%%\n"
        % ((float(num_unique_reads) / float(num_reads_in_reference_sam)) * 100.0)
    )
    ret_lines += (
        "Percent of reads with multiple alignments compared to the reference SAM: %.2f%%\n"
        % (
            (
                float(len(list(sam_reads.keys())) - num_unique_reads)
                / float(num_reads_in_reference_sam)
            )
            * 100.0
        )
    )
    ...
    ret_plot_lines += (
        "percent_reads_with_unique_alignments_in_sam_compared_to_reference_sam\t"
    )
    ret_plot_lines += (
        "percent_reads_with_multiple_alignments_in_sam_compared_to_reference_sam\t"
    )
...
def CollectSAMStats(stats_prefix, suppress_error_messages=False):
    ...
    try:
        ...
        if suppress_error_messages == False:
            print(
                'ERROR: Could not open file "%s" for reading!' % (stats_prefix + ".txt")
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Tool for counting the number of reads and the number of mapped bases in a given SAM file."
        )
        print(
            "Every mapped read is counted only once (even though it might have multiple alignments)."
        )
        print(
            "In case of mapped-base counting, only one alignment (the one with highest MAPQ/AS score)"
        )
        print("is chosen.")
        print(" ")
        print("Usage:")
        print(
            "\t%s <sam_file> [<output_stats_prefix>] [num_reads_in_reference_sam]"
            % sys.argv[0]
        )
        exit(1)
    ...
    [ret_lines, ret_plot_lines] = CountMappedReads(
        sam_file, out_stats_prefix, num_reads_in_reference_sam
    )
    print(ret_lines)
