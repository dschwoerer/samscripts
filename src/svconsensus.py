...
def process_mpileup_line(
    ...
    ret_coverage_sum,
    ret_opened_insertion_events,
    ret_opened_deletion_events,
    coverage_threshold,
    verbose=False,
):
    ...
    for insertion_event in list(insertion_event_counts.keys()):
        if len(insertion_event) > event_length_threshold:
            ...
            ret_opened_insertion_events.append(
                [len(insertion_event), len(insertion_event)]
            )
    ...
    for deletion_event in list(deletion_event_counts.keys()):
        if len(deletion_event) > event_length_threshold:
            ...
            ret_opened_deletion_events.append(
                [len(deletion_event), len(deletion_event)]
            )
...
def process_mpileup(
    ...
    while i < bed_pos_end:  # len(lines)):
        ...
        if thread_id == 0:
            if (j % 1000) == 0:
                sys.stderr.write(
                    "\r[%d] insertion events: %d, deletion events: %d, current coverage: %d"
                    % (i, num_insertion_events, num_deletion_events, int(mpileup_cov))
                )
                sys.stderr.flush()
        ...
        [mpileup_pos, mpileup_cov] = process_mpileup_line(
            line,
            i,
            ret_variant_list,
            ret_vcf_list,
        ...
        if (
            threshold_switch_insertions == False
            and len(opened_insertion_events) >= event_coverage_threshold
        ):
            ##### try:
            ...
            new_insertion_event.max_event_length = max(
                [new_insertion_event.max_event_length]
                + [insertion_event[1] for insertion_event in opened_insertion_events]
            )
        ...
        if (
            threshold_switch_deletions == False
            and len(opened_deletion_events) >= event_coverage_threshold
        ):
            ##### try:
            ...
            new_deletion_event.max_event_length = max(
                [new_deletion_event.max_event_length]
                + [deletion_event[1] for deletion_event in opened_deletion_events]
            )
        ...
        if new_insertion_event.is_open() == True:
            new_insertion_event.max_coverage = max(
                new_insertion_event.max_coverage, len(opened_insertion_events)
            )
            new_insertion_event.max_event_length = max(
                [new_insertion_event.max_event_length]
        ...
        if new_deletion_event.is_open() == True:
            new_deletion_event.max_coverage = max(
                new_deletion_event.max_coverage, len(opened_deletion_events)
            )
            new_deletion_event.max_event_length = max(
                [new_deletion_event.max_event_length]
...
class StructEvent:
    ...
    def verbose_to_string(self):
        event_type = (
            "insertion_in_read"
            if (self.type == "D" or self.type == "insertion_in_read")
            else "deletion_in_read"
        )
        return (
            "Event type: %s, position: %7d, end: %7d, size: %4d, max_event_length = %4d, result: %s, comment: %s"
            % (
                event_type,
                self.position,
...
def chain_events(events_path):
    ...
    for line in fp:
        ...
        event_type = (
            "insertion_in_read"
            if (values[0] == "D" or values[0] == "insertion_in_read")
            else "deletion_in_read"
        )
        start = int(values[1])
        end = int(values[2])
    ...
    for event in sorted_events:
        if len(chained_events) > 0:
            ...
            if distance <= (last_size + event.size) and last_type == event.type:
                ...
                chained_events[-1].max_cov = max(
                    chained_events[-1].max_cov, event.max_cov
                )
                chained_events[-1].max_event_length = max(
                    chained_events[-1].max_event_length, event.max_event_length
                )
                chained_events[-1].size = (
                    event.position + event.size - chained_events[-1].position + 1
                )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        sys.stderr.write("Usage:\n")
        sys.stderr.write(
            "\t%s <reference_file_path> <{sb}am_file> <out_file> [position]\n"
            % sys.argv[0]
        )
        sys.stderr.write(
