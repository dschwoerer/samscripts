...
def get_single_read(fp):
    ...
    if len(header) > 0:
        ...
        header_leading_char = header[0]
        header = header[1:]
        # Strip the '>' or '@' sign from the beginning.
    else:
        return ["", []]
    ...
    if header_leading_char == ">":
        ...
        sys.stderr.write(
            'ERROR: Sequence is not in FASTA nor FASTQ format! Leading character in the header is "%s". Exiting.\n'
            % (header_leading_char)
        )
        exit(1)
    ...
    while num_loaded_lines < num_expected_lines and len(next_char) > 0:
        ...
        if header_leading_char == "@":
            if (
                (num_loaded_lines == 1 and next_char == "+")
                or (num_loaded_lines == 2 and line[0] == "+")
                or (num_loaded_lines == 3 and next_char == sequence_separator)
            ):
                line_string += line.rstrip()
                lines.append(line_string)
                num_loaded_lines += 1
                ...
                sys.stderr.write(
                    'ERROR: File is not in FASTQ format! Separator between seq and qual is not "+" but "%s". Exiting.\n'
                    % (line[0])
                )
                exit(1)
            else:
...
def count_seq_length(fastq_path):
    ...
    if len(all_read_lengths) > 0:
        ...
        median_seq_len = 0
        sys.stderr.write(
            'Warning: No sequences were loaded from file "%s" (function: fastqparser.count_seq_length).\n'
            % (fastq_path)
        )
        sys.stderr.flush()
